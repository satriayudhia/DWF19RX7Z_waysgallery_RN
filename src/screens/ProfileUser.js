...
const Profile = (props) => {
  ...
  const getUser = async () => {
    try {
      console.log("props Detail nav:", props);
      const getUserLogin = await AsyncStorage.getItem("userInfo");
      const parsing = await JSON.parse(getUserLogin);
      setUserLogin(parsing);

      const res = await API.get(`/user-profile/${props.route.params}`);
  ...
  const handleFollow = async () => {
    try {
      const body = { following: user.id, follower: userLogin.id };
  ...
  return user == undefined || userLogin == undefined ? (
    <AppLoading />
  ) : (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.btnContainer}>
          ...
          {user.following.find((follow) => follow.following === user.id) ? (
            <Button
              style={{ width: 150 }}
              mode="contained"
              color="#6d747e"
        ...
        <View style={{ marginTop: 20 }}>
          ...
          {user.post.length > 0 &&
            user.post.slice(0, 5).map((post) => (
              <TouchableNativeFeedback
                key={post.id}
                onPress={() => props.navigation.navigate("Detail", post.id)}
              >
